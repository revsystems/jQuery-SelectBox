...
*/
(function($){jQuery.fn.borderWidth=function(){return $(this).outerWidth()-$(this).innerWidth();};jQuery.fn.marginWidth=function(){return $(this).outerWidth(true)-$(this).outerWidth();};jQuery.fn.paddingWidth=function(){return $(this).innerWidth()-$(this).width();};jQuery.fn.extraWidth=function(){return $(this).outerWidth(true)-$(this).width();};jQuery.fn.offsetFrom=function($e){return{left:$(this).offset().left-$e.offset().left,top:$(this).offset().top-$e.offset().top};};jQuery.fn.maxWidth=function(){var max=0;$(this).each(function(){if($(this).width()>max)max=$(this).width();});return max;};jQuery.fn.sb=function(o){if($.browser.msie&&$.browser.version<7)return $(this);o=$.extend({acTimeout:800,animDuration:300,ddCtx:'body',dropupThreshold:150,fixedWidth:false,maxHeight:false,maxWidth:false,selectboxClass:'selectbox',useTie:false,arrowMarkup:"<span class='arrow_btn'><span class='interior'><span class='arrow'></span></span></span>",displayFormat:function(){if($(this).size()>0){var label=$(this).attr("label");if($.trim(label)!="")return label;return $(this).text();}
else{return"";}},optionFormat:function(ogIndex,optIndex){var label=$(this).attr("label");if($.trim(label)!="")return label;return $(this).text();},optgroupFormat:function(ogIndex){return"<span class='label'>"+$(this).attr("label")+"</span>";}},o);$(this).each(function(){var $orig=$(this);var $sb=null;var $display=null;var $dd=null;var $items=null;if($orig.hasClass("has_sb")){return;}
else{$orig.addClass("has_sb");}
function loadSB(){var optionMarkup=$orig.children().size()>0?o.displayFormat.call($orig.find("option:selected")[0],0,0):"&nbsp;";$sb=$("<div class='sb "+o.selectboxClass+" "+$orig.attr("class")+"'></div>");$("body").append($sb);$display=$("<a href='#' class='display "+$orig.attr("class")+"'><span class='value'>"+$orig.val()+"</span> <span class='text'>"+optionMarkup+"</span>"+o.arrowMarkup+"</a>");$sb.append($display);$dd=$("<ul class='"+o.selectboxClass+" items "+$orig.attr("class")+"'></ul>");$sb.append($dd);if($orig.children().size()==0){var $emptyLi=$("<li class='selected empty'><a href='#'><span class='value'></span><span class='text'>&nbsp;</span></a></li>");$emptyLi.data("val","");$dd.append($emptyLi);}
else{$orig.children().each(function(i){if($(this).is("optgroup")){var $og=$(this);var $ogItem=$("<li class='optgroup'>"+o.optgroupFormat.call($og[0],i+1)+"</li>");var $ogList=$("<ul class='items'></ul>");$ogItem.append($ogList);$dd.append($ogItem);$og.children("option").each(function(j){var $li=$("<li class='"+($(this).attr("selected")?"selected":"")+" "+($(this).attr("disabled")?"disabled":"")+"'><a href='#'><span class='value'>"+$(this).attr("value")+"</span><span class='text'>"+o.optionFormat.call(this,0,i+1)+"</span></a></li>");$li.data("val",$(this).attr("value"));$ogList.append($li);});}
else{var $li=$("<li class='"+($(this).attr("selected")?"selected":"")+" "+($(this).attr("disabled")?"disabled":"")+"'><a href='#'><span class='value'>"+$(this).attr("value")+"</span><span class='text'>"+o.optionFormat.call(this,0,i+1)+"</span></a></li>");$li.data("val",$(this).attr("value"));$dd.append($li);}});}
...
function destroySB(){$sb.unbind().find("*").unbind();$sb.remove();$orig.unbind("reload",reloadSB).unbind("domupdate",delayReloadSB).unbind("focus",focusOrig).removeClass("has_sb").show();}
function killAndUnbind(){killAll();$(document).unbind("click",killAndUnbind);}
function killAll(){$(".sb."+o.selectboxClass).each(function(){$(this).triggerHandler("close");});}
function killAllButMe(){$(".sb."+o.selectboxClass).not($sb[0]).each(function(){$(this).triggerHandler("close");});}
function closeSB(){if($sb.is(".open")){$items.removeClass("hover");$(document).unbind("keyup",keyupSB);$(document).unbind("keydown",stopPageHotkeys);$(document).unbind("keydown",keydownSB);$dd.fadeOut(o.animDuration,function(){$sb.removeClass("open");$sb.append($dd);});}}
...
function positionSB(){var $ddCtx=getDDCtx();var ddMaxHeight=0;var ddX=$display.offsetFrom($ddCtx).left;var ddY=0;var dir="";$dd.removeClass("above");$dd.css({display:"block",maxHeight:"none",position:"relative",visibility:"hidden"});if(!o.fixedWidth){$dd.width($display.outerWidth()-$dd.extraWidth()+1);}
...
var bodyX=$().jquery<"1.4.2"?$("body").offset().left:parseInt($("body").css("margin-left"));var bodyY=$().jquery<"1.4.2"?$("body").offset().top:parseInt($("body").css("margin-top"));$dd.css({display:"none",left:ddX+($ddCtx[0].tagName.toLowerCase()=="body"?bodyX:0),maxHeight:ddMaxHeight,position:"absolute",top:ddY+($ddCtx[0].tagName.toLowerCase()=="body"?bodyY:0),visibility:"visible"});if(dir=="up")$dd.addClass("above");return dir;}
function clickSB(e){var $sb=$(this).closest("."+o.selectboxClass);if($sb.is(".open")){closeSB();}
else{$display.focus();openSB();}
return false;}
function selectItem(){var $item=$(this);$display.find(".value").html($item.find(".value").html());$display.find(".text").attr("title",$item.find(".text").html());$dd.find("li").removeClass("selected");$item.closest("li").addClass("selected");var oldVal=$orig.val();var newVal=$item.closest("li").data("val");$orig.val(newVal);$display.find(".text").html(o.displayFormat.call($orig.find("option:selected")[0]));if(oldVal!=newVal){$orig.change();}}
...
function selectNextItemStartsWith(c){var $selected=$items.filter(".selected:first");var st=$selected.find(".text").text().toLowerCase();var $available=$items.not(".disabled");for(var i=$items.not(".disabled").index($selected)+1;i<$available.size();i++){var t=$available.eq(i).find(".text").text();if(t!=""&&t.substring(0,1).toLowerCase()==c.toLowerCase()){selectItem.call($available.eq(i)[0]);return true;}}
return false;}
...
break;default:break;}}
function keyupSB(e){if(e.altKey||e.ctrlKey)return false;var $selected=$items.filter(".selected");if(e.which!=38&&e.which!=40){searchTerm+=String.fromCharCode(e.keyCode);if(searchTerm.length>0&&selectMatchingItem(searchTerm)){clearTimeout(cstTimeout);cstTimeout=setTimeout(clearSearchTerm,o.acTimeout);}
else if(selectNextItemStartsWith(String.fromCharCode(e.keyCode))){clearTimeout(cstTimeout);cstTimeout=setTimeout(clearSearchTerm,o.acTimeout);}
else{clearTimeout(cstTimeout);clearSearchTerm();}}}
